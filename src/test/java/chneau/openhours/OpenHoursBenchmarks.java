...
public class OpenHoursBenchmarks {
    ...
    public static void runBenchmarks(PrintStream out) throws Exception {
        ...
        out.printf("1. IsOpen (100k rolling calls):            %4.0f ms (%.3f us/op)%n", d1, ((t1 - t0) / 1000.0) / (iterations * 10));
        ...
        out.printf("2. IsOpen (1M pure calls):                 %4.0f ms (%.3f us/op)%n", d2, ((t1 - t0) / 1000.0) / 1_000_000.0);
        ...
        out.printf("3. GetTimeToOpen (10k calls):              %4.0f ms (%.3f us/op)%n", d3, ((t1 - t0) / 1000.0) / iterations);
        ...
        out.printf("4. GetTimeToOpenForDuration 4h (10k calls):%4.0f ms (%.3f us/op)%n", d4, ((t1 - t0) / 1000.0) / iterations);
        ...
        out.printf("5. When 4h (10k calls):                    %4.0f ms (%.3f us/op)%n", d5, ((t1 - t0) / 1000.0) / iterations);
        ...
        out.printf("6. NextDur (10k calls):                    %4.0f ms (%.3f us/op)%n", d6, ((t1 - t0) / 1000.0) / iterations);
        ...
        out.printf("7. NextDate (10k calls):                   %4.0f ms (%.3f us/op)%n", d7, ((t1 - t0) / 1000.0) / iterations);
        ...
        out.printf("8. Parse Cached (1k calls):                %4.0f ms (%.3f us/op)%n", d8, ((t1 - t0) / 1000.0) / 1000.0);
        ...
        out.printf("9. JSON Deserialize (1k calls):            %4.0f ms (%.3f us/op)%n", d9, ((t1 - t0) / 1000.0) / 1000.0);

        // 10. Simulation Stress Test (5,000 unique objects)
        t0 = System.nanoTime();
        List<OpenHours> locations = new ArrayList<>(5000);
        ...
        out.printf("10. Stress Test (5,000 unique objects):    %4.0f ms (%.4f ms/obj)%n", d10, d10 / 5000.0);
        out.println("========================================================");
    }
}
